...
//				if (res.size() == 0)
			...
			} else {// 수가 같지 않을 때 : -1로 장애물이 있음을 표시
				if (!res.isEmpty() && res.lastElement() != -1) {// -1이 여러개 있으면 보기 불편하니까 하나만.
					res.add(-1);
				}
			}
			i = p.y;
...
//			if (arr[a][i]==0) continue; 없어도 잘 돌아간다. 
			...
			if (p.x == arr[i][a]) {
				if (!res.isEmpty()) {
					if (!res.isEmpty() && res.lastElement() != i) {
	...
	public static void moveLeft(int arr[][]) {
		for (int i = 0; i < arr.length; ++i) {
			...
			for (int j = 0; j + 1 < SIZE;) {// 합쳐지는 녀석들을 겹친다.
				if (vec.get(j) == -1) {// 끊겼을 때
					++j;
					continue;
				}
				// 같다는 것은 이미 getRowVec에서 검사함.
				if (vec.get(j) != -1 && vec.get(j + 1) != -1) {
	...
	public static void moveDown(int arr[][]) {
		for (int i = 0; i < arr.length; ++i) {
			...
			for (int j = 0; j + 1 < SIZE;) {// 합쳐지는 녀석들을 겹친다.
				...
				// 같다는 것은 이미 getRowVec에서 검사함. 중간에 -1이 있으면 장애물이 있어서 합쳐지지 못함
				if (vec.get(j) != -1 && vec.get(j + 1) != -1) {
...
//		int ans = 0;
//		for (int i = 0; i < 4; ++i) {
//			int copy1[][] = getClone(arr);
//			move(copy1, dir[i]);
//			for (int j = 0; j < 4; j++) {
//				int copy2[][] = getClone(copy1);
//				move(copy2, dir[i]);
//				for (int k = 0; k < 4; k++) {
//					int copy3[][] = getClone(copy2);
//					move(copy3, dir[i]);
//					for (int l = 0; l < 4; l++) {
//						int copy4[][] = getClone(copy3);
//						move(copy4, dir[i]);
//						for (int m = 0; m < 4; m++) {
//							int copy5[][] = getClone(copy4);
//							move(copy5, dir[i]);
//							int temp = getMax(copy5);
